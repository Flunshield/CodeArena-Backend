...
export class AuthController {
    ...
    async refreshAccessToken(@Req() request, @Res() response): Promise<void> {
        try {
            const refreshToken = request.cookies['frenchcodeareatoken'];
            const accessToken = await this.refreshTokenService.generateAccessTokenFromRefreshToken(
                refreshToken,
            );
            if (!accessToken) {
                ...
                try {
                    ...
                    throw new HttpException('Merci de vous authentifier', HttpStatus.INTERNAL_SERVER_ERROR);
                }
            }
        } catch (error: any) {
            throw new HttpException('Merci de vous authentifier', HttpStatus.INTERNAL_SERVER_ERROR);
